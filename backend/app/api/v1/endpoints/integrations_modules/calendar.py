...
class ConflictResolutionRequest(BaseModel):
    conflict_id: str
    resolution_action: str = Field(..., pattern="^(keep_event1|keep_event2|merge_events|manual)$")
    ...
    try:
        ...
        if result.get("status") != "completed":
            raise HTTPException(
                ...
                detail=f"Failed to create calendar event: {result.get('error', 'Unknown error')}"
            )
        
        event_data = result.get("result", {}).get("event", {})
...
async def update_calendar_event(
    ...
    try:
        ...
        if result.get("status") != "completed":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Failed to update calendar event: {result.get('error', 'Unknown error')}"
            )
...
async def delete_calendar_event(
    ...
    try:
        ...
        if result.get("status") != "completed":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Failed to delete calendar event: {result.get('error', 'Unknown error')}"
            )
