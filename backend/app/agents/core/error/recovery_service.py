...
from ..state.state_manager import WorkflowStateManager, StateStatus, StateSnapshot
...
class WorkflowRecoveryService:
    ...
    def __init__(self):
        ...
        # Recovery scheduler will be started when needed
    ...
    def _start_recovery_scheduler(self):
        ...
        if self.recovery_scheduler_task is None or self.recovery_scheduler_task.done():
            try:
                self.recovery_scheduler_task = asyncio.create_task(self._recovery_scheduler_loop())
                logger.info("Started workflow recovery scheduler")
            except RuntimeError as e:
                logger.warning(f"Could not start recovery scheduler: {e}")
    
    async def start_recovery_scheduler(self):
        """Start the recovery scheduler task (safe to call from async context)"""
        if self.recovery_scheduler_task is None or self.recovery_scheduler_task.done():
            self.recovery_scheduler_task = asyncio.create_task(self._recovery_scheduler_loop())
            logger.info("Started workflow recovery scheduler")
    
    async def stop_recovery_scheduler(self):
        """Stop the recovery scheduler task"""
        if self.recovery_scheduler_task and not self.recovery_scheduler_task.done():
            self.recovery_scheduler_task.cancel()
            try:
                await self.recovery_scheduler_task
            except asyncio.CancelledError:
                pass
            logger.info("Stopped workflow recovery scheduler")
