            ...
            if self.execution_context:
                ...
                logger.info(
                    ...
                    extra={
                        ...
                        "tools_used": len(self.execution_context.active_tools)
                    }
                )
        ...
        while not self.cancelled:
            try:
                ...
                if self.execution_context:
                    ...
                    # Check tool limits
                    if len(self.execution_context.active_tools) > self.resource_limits.max_concurrent_tools:
                        logger.warning(
                            f"Workflow exceeding concurrent tool limit",
                            extra={
                                "workflow_id": self.execution_context.workflow_id,
                                "active_tools": len(self.execution_context.active_tools),
                                "limit": self.resource_limits.max_concurrent_tools
                            }
                        )
                        
                        # Cancel workflow if severely over limit
                        if len(self.execution_context.active_tools) > self.resource_limits.max_concurrent_tools * 1.5:
                            await self.cancel()
                            break
...
class WorkflowContainerFactory:
    ...
    def create_container(
        ...
        limits_map = {
            ...
            WorkflowType.DATABASE: WorkflowResourceLimits(
                max_execution_time=30.0,  # 30 seconds for database ops
                max_memory_mb=64,
