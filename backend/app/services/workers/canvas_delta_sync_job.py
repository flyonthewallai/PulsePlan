...
class CanvasDeltaSyncJob:
    ...
    async def _get_last_delta_sync(self, user_id: str) -> Optional[datetime]:
        """Get timestamp of last delta sync from oauth_tokens metadata"""
        try:
            response = await self.supabase.table("oauth_tokens").select(
                "metadata"
            ).eq("user_id", user_id).eq("provider", "canvas").single().execute()
            ...
            last_delta = metadata.get("last_delta_at")
            last_full = metadata.get("last_full_sync_at")
    ...
    async def _update_last_delta_sync(self, user_id: str, sync_timestamp: datetime):
        """Update last delta sync timestamp in oauth_tokens metadata"""
        try:
            # Get current metadata
            response = await self.supabase.table("oauth_tokens").select("metadata").eq(
                "user_id", user_id
            ).eq("provider", "canvas").single().execute()

            metadata = response.data.get("metadata", {}) if response.data else {}
            metadata["last_delta_at"] = sync_timestamp.isoformat()

            await self.supabase.table("oauth_tokens").update({
                "metadata": metadata,
                "updated_at": datetime.utcnow().isoformat()
    ...
    async def _update_integration_status(self, user_id: str):
        """Update integration status to OK in oauth_tokens metadata"""
        try:
            # Get current metadata
            response = await self.supabase.table("oauth_tokens").select("metadata").eq(
                "user_id", user_id
    ...
    async def _create_error_result(
        ...
        if "401" in error_message or "unauthorized" in error_message.lower():
            ...
            # Mark as error for other failures in oauth_tokens metadata
            try:
                response = await self.supabase.table("oauth_tokens").select("metadata").eq(
                    "user_id", user_id
                ).eq("provider", "canvas").single().execute()
                ...
                metadata["last_error_code"] = "delta_sync_failed"
