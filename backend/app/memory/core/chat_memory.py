...
from app.config.cache.redis_client import get_redis_client, RedisClient
...
class ChatMemoryService:
    ...
    async def get_recent_turns(
        ...
        try:
            ...
            raw_turns = await self.redis_client.lrange(key, 0, limit - 1)
    ...
    async def get_session_stats(self, user_id: str, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            session_length = await self.redis_client.llen(key)
    ...
    async def get_recent_conversation_turns(
        ...
        """Get recent conversation turns - alias for get_recent_turns"""
        return await self.get_recent_turns(user_id, conversation_id, limit)
    ...
    async def get_conversation_summary(
        ...
        try:
            turns = await self.get_recent_turns(user_id, conversation_id, limit=10)
            if not turns:
                return ""

            # Create a simple summary from recent turns
            summary_lines = []
