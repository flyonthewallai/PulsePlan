...
class CanvasBackfillJob:
    ...
    async def _get_last_full_sync(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.data:
                return {
                    "last_full_sync_at": response.data.get("last_full_sync_at"),
                    "completed": response.data.get("last_full_sync_at") is not None
                }
            return None
    ...
    async def _mark_backfill_completed(self, user_id: str, results: Dict[str, Any]):
        """Mark backfill as completed in integration record"""
        try:
